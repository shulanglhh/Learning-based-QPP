...
from src.plan_encoding.encoding_datatype import encoding_datatype
...
def obtain_upper_bound_query_size(path):
    ...
    condition_max_num = 0
    plan_row_max = 0
    plan_row_min = 9999999999
    plan_width_max = 0
    plan_width_min = 9999999
    cost_label_max = 0.0
        ...
        for plan in f.readlines():
            ...
            for node in sequence:
                if node == None:
                    continue
                if 'plan_row' in node:
                    plan_row = node['plan_row']
                    plan_width = node['plan_width']
                    if plan_row > plan_row_max:
                        plan_row_max = plan_row
                    elif plan_row < plan_row_min:
                        plan_row_min = plan_row
                    if plan_width > plan_width_max:
                        plan_width_max = plan_width
                    elif plan_width < plan_width_min:
                        plan_width_min = plan_width
    ...
    return plan_node_max_num, condition_max_num, cost_label_min, cost_label_max, \
           plan_row_max, plan_row_min, plan_width_max, plan_width_min
...
def prepare_dataset(database):
    ...
    index2column = {'aka_name_pkey': 'id', 'aka_title_pkey': 'id', 'cast_info_pkey': 'id', 'char_name_pkey': 'id',
                    'comp_cast_type_pkey': 'id', 'company_name_pkey': 'id', 'company_type_pkey': 'id',
                    'complete_cast_pkey': 'id',
                    'info_type_pkey': 'id', 'keyword_pkey': 'id', 'kind_type_pkey': 'id', 'link_type_pkey': 'id',
                    'movie_companies_pkey': 'id',
                    'movie_info_idx_pkey': 'id', 'movie_keyword_pkey': 'id', 'movie_link_pkey': 'id', 'name_pkey': 'id',
                    'role_type_pkey': 'id',
                    'title_pkey': 'id', 'movie_info_pkey': 'id', 'person_info_pkey': 'id',
                    'company_id_movie_companies': 'company_id', 'company_type_id_movie_companies': 'company_type_id',
                    'info_type_id_movie_info': 'info_type_id', 'info_type_id_movie_info_idx': 'info_type_id',
                    'info_type_id_person_info': 'info_type_id', 'keyword_id_movie_keyword': 'keyword_id',
                    'kind_id_aka_title': 'kind_id', 'kind_id_title': 'kind_id',
                    'link_type_id_movie_link': 'link_type_id', 'linked_movie_id_movie_link': 'linked_movie_id',
                    'movie_id_aka_title': 'movie_id', 'movie_id_cast_info': 'movie_id',
                    'movie_id_complete_cast': 'movie_id',
                    'movie_id_movie_companies': 'movie_id', 'movie_id_movie_info': 'movie_id',
                    'movie_id_movie_info_idx': 'movie_id',
                    'movie_id_movie_keyword': 'movie_id', 'movie_id_movie_link': 'movie_id',
                    'person_id_aka_name': 'person_id',
                    'person_id_cast_info': 'person_id', 'role_id_cast_info': 'role_id'}
    ...
    column_id = 1
    for table_name in tables:
        table_vec[table_name] = table_datatype_vec[table_name]
        tables_id[table_name] = table_id
        table_id += 1
        for column in column2pos[table_name]:
            columns_id[table_name + '.' + column] = column_id
            columns_vec[table_name + '.' + column] = column_datatype_vec[column]
            column_id += 1

    return column2pos, indexes_id, tables_id, table_vec, columns_id, column_datatype_vec, columns_vec, physic_ops_id, compare_ops_id, bool_ops_id, tables, index2column
