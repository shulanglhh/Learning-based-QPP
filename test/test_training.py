...
class TestTraining(unittest.TestCase):
    def test(self):
        ...
        plan_node_max_num, condition_max_num, cost_label_min, cost_label_max, plan_row_max, plan_row_min, plan_width_max, plan_width_min = obtain_upper_bound_query_size(
            '/home/lhh/Learning-based-QPP/test_files_open_source/plans_seq.json')
        print('query upper size prepared')

        parameters = Parameters(condition_max_num, indexes_id, tables_id, table_vec, columns_id, column_datatype_vec,
                                column_vec, physic_ops_id, column_total_num,
                                table_total_num, type_total_number, index_total_num, physic_op_total_num,
                                condition_op_dim, compare_ops_id, bool_ops_id,
                                bool_ops_total_num, compare_ops_total_num, dataset, min_max_column, word_vectors,
                                cost_label_min,
                                cost_label_max, plan_row_max, plan_row_min, plan_width_max, plan_width_min,
                                index2column)

        train(0, 10000, 10000, 12000, 10, parameters, '/home/lhh/Learning-based-QPP/test_files_open_source/job')
